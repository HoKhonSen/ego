...
public class TbItemParamServiceImpl implements TbItemParamService{
	@Reference
	private TbItemParamDubboService tbItemParamServiceImpl;
	...
	public EgoResult showParam(long catId) {
		...
		TbItemParam param = tbItemParamServiceImpl.selByCatId(catId);
		if (param!=null) {
			er.setStatus(200);
			er.setData(param);
		}
	...
	public EgoResult save(TbItemParam param) {
		...
		int index = tbItemParamServiceImpl.insParam(param);
		EgoResult er = new EgoResult();
		if (index == 1) {
			er.setStatus(200);
		}
